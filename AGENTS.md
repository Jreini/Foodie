...
- `FoodieApp.swift` — app entry; injects `AuthManager` into the environment (and a Core Data context no feature uses).
- `Views/RootView.swift` — switches on `AuthManager.state`; only `.ready` reaches `MainTabView`.
- `Models/` — plain structs: `User`, `Profile`, `Friendship` (+ `FriendshipState`), `Restaurant`, `Review`, `TastingListEntry`, `SharedList`, `FriendActivity`, `RestaurantTier`.
- `ViewModels/` — one per screen: `FeedViewModel`, `DiscoverViewModel`, `DecisionEngineViewModel`, `TastingListViewModel`, `ProfileViewModel`, `UserProfileViewModel`, `FriendsViewModel`, `SharedListsViewModel`, `SharedListDetailViewModel`.
- `Views/` — grouped by feature: `Auth/`, `Feed/`, `Discover/`, `Decide/`, `Lists/`, `Map/`, `Friends/`, `Profile/`, plus shared `Components/` (star ratings, tier badges/slider, chips, rows, `ProfileImageView`, `CameraPicker`).
...
- `Services/` — `DataServiceProtocol` + its two implementations, `AuthManager`, `ProfileService`, `SupabaseService`, `PhotoUploadService`, `PlaceSearchService`, `LocationProvider`, `TierFlaggingService`.
